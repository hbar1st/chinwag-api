...
describe("user tests", () => {
  ...
  describe("Delete User", () => {
    ...
    beforeAll(async () => {
      ...
      vi.unmock("cloudinary"); // removes the mock
      cloudinary = await import("cloudinary");
      const Image = (await import("../src/utils/Image.js")).default;
      ...
      //user = res.body.data;
      logger.info("signed up as: ", res.body.data);
    ...
    test("Delete user", async () => {
      //get the user info first to facilitate checking 
      const user = await request(app)
      .get(`${route}`)
      .set("Authorization", bearerToken);
      
      expect(user.status).toEqual(200);
      expect(user.body.data.public_id).toBeDefined();
      const ids = [user.body.data.public_id];
      ...
      let sql = `SELECT * FROM chinwag.users WHERE username='${user.username}';`;

      // start by seeding the table first
      let { rows } = await pool.query(sql);
      expect(rows.length).toBe(0);

      sql = `SELECT * from chinwag.images WHERE public_id='${ids[0]}';`;
      
      rows = (await pool.query(sql)).rows;
      expect(rows.length).toBe(0);

      const images = await cloudinary.v2.api.resources_by_ids(ids);
      expect(images.resources.length).toBe(0);
    });
  });
